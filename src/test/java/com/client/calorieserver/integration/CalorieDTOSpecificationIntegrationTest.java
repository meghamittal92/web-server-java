...
import com.client.calorieserver.util.search.CalorieDTOSpecification;
...
public class CalorieDTOSpecificationIntegrationTest {
	...
	public void queryNumberStringSuccess(final Filter filter, final Integer resultSize) {
		...
		Assertions.assertTrue(results.size() == resultSize,
				String.format("Result Size expected %d and actual %d", resultSize, results.size()));
	...
	private static Stream<Arguments> providerNumberStringQuery() {
		return Stream.of(
				...
				Arguments.of(new Filter(CalorieSearchKey.numCalories.getName(), RelationalOperator.GREATER_THAN, 50),
						1),
				Arguments.of(new Filter(CalorieSearchKey.numCalories.getName(),
						RelationalOperator.GREATER_THAN_EQUAL_TO, 40), 2),
				Arguments.of(new Filter(CalorieSearchKey.numCalories.getName(), RelationalOperator.LESS_THAN, 40), 0),
				Arguments.of(
						new Filter(CalorieSearchKey.numCalories.getName(), RelationalOperator.LESS_THAN_EQUAL_TO, 40),
						1),
	...
	private static Stream<Arguments> providerDateTimeQuery() {
		return Stream.of(
				Arguments.of(new Filter(CalorieSearchKey.dateTime.getName(), RelationalOperator.GREATER_THAN,
						localDateTime1), 1),
				Arguments.of(new Filter(CalorieSearchKey.dateTime.getName(), RelationalOperator.GREATER_THAN_EQUAL_TO,
						localDateTime1), 2),
				Arguments.of(
						new Filter(CalorieSearchKey.dateTime.getName(), RelationalOperator.LESS_THAN, localDateTime1),
						0),
				Arguments.of(new Filter(CalorieSearchKey.dateTime.getName(), RelationalOperator.LESS_THAN_EQUAL_TO,
						localDateTime1), 1),
				Arguments.of(new Filter(CalorieSearchKey.dateTime.getName(), RelationalOperator.EQUAL, localDateTime2),
						1),
				Arguments.of(
						new Filter(CalorieSearchKey.dateTime.getName(), RelationalOperator.NOT_EQUAL, localDateTime2),
						1));
	}
	...
	private static Stream<Arguments> providerBooleanQuery() {
		return Stream.of(
				...
				Arguments.of(new Filter(CalorieSearchKey.withinLimit.getName(), RelationalOperator.NOT_EQUAL, false),
						1));
	}
	...
	private static Stream<Arguments> providerQueryInvalidQueryException() {
		return Stream.of(Arguments.of(UNSUPPPORTED_KEY, RelationalOperator.EQUAL.getName(), true),
	...
	public void queryStringSuccess() {
		SEARCH_STRING = separateTokensBySpaces(Bracket.LEFT_PARANTHESIS.getName(),
				CalorieSearchKey.numCalories.getName() + RelationalOperator.GREATER_THAN.getName()
						+ NUM_CALORIES_GREATER_THAN_VALUE,
				LogicalOperator.AND.name(),
				CalorieSearchKey.dateTime.getName() + RelationalOperator.LESS_THAN.getName() + DATETIME_STRING,
				Bracket.RIGHT_PARANTHESIS.getName(), LogicalOperator.OR.name(),
		...
		Assertions.assertTrue(results.size() == 2,
				String.format("Result Size expected %d and actual %d", 2, results.size()));
	...
	private static Stream<Arguments> providerQueryWithStringInvalidQueryException() {
		return Stream.of(Arguments.of(INVALID_SEARCH_STRING),
