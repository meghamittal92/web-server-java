...
public class CalorieControllerTest extends BaseIntegrationTest {
	...
	void find() throws Exception {
		...
		result = mockMvc
				.perform(MockMvcRequestBuilders.get(caloriesRequestPath + "/" + calorieID)
						.contentType(MediaType.APPLICATION_JSON).header(HttpHeaders.AUTHORIZATION, token))
				.andExpect(status().is2xxSuccessful()).andReturn();
		jsonResult = new JSONObject(result.getResponse().getContentAsString());
		assert jsonResult.getInt("id") == calorieID;
	...
	void findAll() throws Exception {
		...
		mockMvc.perform(post(caloriesRequestPath).contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(params)).header(HttpHeaders.AUTHORIZATION, token))
				.andExpect(status().is2xxSuccessful()).andReturn();
		params = new HashMap<>();
		params.put("numCalories", "5");
		params.put("dateTime", LocalDateTime.now().plusSeconds(10).truncatedTo(ChronoUnit.SECONDS)
		...
		params = new HashMap<>();
		params.put("numCalories", "10");
		params.put("dateTime", LocalDateTime.now().plusDays(1).truncatedTo(ChronoUnit.SECONDS)
				.format(DateTimeFormatter.ISO_DATE_TIME));
		params.put("userId", userId.toString());
		...
		params = new HashMap<>();
		params.put("numCalories", "10");
		params.put("dateTime", LocalDateTime.now().plusDays(1).plusSeconds(5).truncatedTo(ChronoUnit.SECONDS)
				.format(DateTimeFormatter.ISO_DATE_TIME));
		params.put("userId", userId.toString());
		...
		params.put("dateTime", LocalDateTime.now().plusDays(1).plusSeconds(10).truncatedTo(ChronoUnit.SECONDS)
				.format(DateTimeFormatter.ISO_DATE_TIME));
		params.put("userId", userId.toString());
		...
		params.put("dateTime", LocalDateTime.now().plusDays(3).truncatedTo(ChronoUnit.SECONDS)
				.format(DateTimeFormatter.ISO_DATE_TIME));
		params.put("userId", userId.toString());
		...
		params.put("dateTime", LocalDateTime.now().plusDays(3).plusSeconds(2).truncatedTo(ChronoUnit.SECONDS)
				.format(DateTimeFormatter.ISO_DATE_TIME));
		params.put("userId", userId.toString());
		...
		MvcResult result = mockMvc.perform(get(caloriesRequestPath).contentType(MediaType.APPLICATION_JSON)
				.header(HttpHeaders.AUTHORIZATION, token)).andExpect(status().is2xxSuccessful()).andReturn();
		JSONObject jsonObject = new JSONObject(result.getResponse().getContentAsString());
		assert (jsonObject.getJSONArray("content").length() == 7);

		result = mockMvc
				.perform(get(caloriesRequestPath).contentType(MediaType.APPLICATION_JSON)
		...
		result = mockMvc.perform(get(caloriesRequestPath).contentType(MediaType.APPLICATION_JSON)
