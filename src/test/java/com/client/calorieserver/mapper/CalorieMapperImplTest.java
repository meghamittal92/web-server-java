...
public class CalorieMapperImplTest {
	...
	private static final LocalDateTime calorie_time = LocalDateTime.now().truncatedTo(ChronoUnit.SECONDS);
	...
	void afterMappingCalorie() {
		...
		calorieDTO.getUserDTO().setExpectedCaloriesPerDay(TestData.getDefaultCaloriePerDayDTO().getTotalCalories());
		...
		calorieDTO.getUserDTO().setExpectedCaloriesPerDay(calorieDTO.getNumCalories() / 2);
		calorieDTO.setCaloriePerDayDTO(TestData.getDefaultCaloriePerDayDTO());
	...
	void afterMappingCalorieCreateCalorieRequest() {
		...
		Mockito.when(calorieAccessor.getCalories(Mockito.anyString(), Mockito.anyLong())).thenReturn(100);
