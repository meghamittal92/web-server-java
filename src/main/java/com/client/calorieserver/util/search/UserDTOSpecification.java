...
public class UserDTOSpecification implements Specification<UserDTO> {
	...
	@Override
	public Predicate toPredicate(final Root<UserDTO> root, final CriteriaQuery<?> query,
		...
		try {
			...
			case EQUAL:
				return builder.equal(root.get(userSearchKey.getDbColumnName()), castToRequiredType(
						root.get(userSearchKey.getDbColumnName()).getJavaType(), criteria.getValue()));
			case NOT_EQUAL:
				return builder.notEqual(root.get(userSearchKey.getDbColumnName()), castToRequiredType(
						root.get(userSearchKey.getDbColumnName()).getJavaType(), criteria.getValue()));
			case GREATER_THAN:
				return builder.greaterThan(root.get(userSearchKey.getDbColumnName()),
						castToRequiredType(root.get(userSearchKey.getDbColumnName()).getJavaType(), criteria.getValue())
								.toString());
			case LESS_THAN:
				return builder.lessThan(root.get(userSearchKey.getDbColumnName()),
						castToRequiredType(root.get(userSearchKey.getDbColumnName()).getJavaType(), criteria.getValue())
								.toString());
			case GREATER_THAN_EQUAL_TO:
				return builder.greaterThanOrEqualTo(root.get(userSearchKey.getDbColumnName()),
						castToRequiredType(root.get(userSearchKey.getDbColumnName()).getJavaType(), criteria.getValue())
								.toString());
			case LESS_THAN_EQUAL_TO:
				return builder.lessThanOrEqualTo(root.get(userSearchKey.getDbColumnName()),
						castToRequiredType(root.get(userSearchKey.getDbColumnName()).getJavaType(), criteria.getValue())
								.toString());
			case LIKE:
				return builder.like(root.get(userSearchKey.getDbColumnName()),
						castToRequiredType(root.get(userSearchKey.getDbColumnName()).getJavaType(),
