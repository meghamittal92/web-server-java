...
/**
 * Generic builder class to build JPA Specification query for any provided class using its
 * Specification class.
 *
 * @param <U>
 */
public class SpecificationBuilder<U> {
	...
	private static final Pattern SpecCriteriaRegex = Pattern.compile(
			"^([a-zA-Z_0-9\\-]+?)(" + Joiner.on("|").join(RelationalOperator.getNames()) + ")([a-zA-Z_0-9\\-:@]+?)$");
	...
	public Specification<U> build(Function<Filter, Specification<U>> converter) {
		...
		Specification specification = specStack.isEmpty() ? (specs.isEmpty() ? null : Specification.where(specs.get(0)))
				: specStack.pop();
			...
			Arrays.stream(searchString.split("\\s+")).forEach(token -> {
				if (LogicalOperator.get(token) != null) {
					...
					while (!(stack.peek() instanceof Bracket
							&& Bracket.isMatchingParen((Bracket) stack.peek(), rightParen)))
					...
					if (matcher.find()) {
						postFixExpStack.push(new Filter(matcher.group(1), RelationalOperator.get(matcher.group(2)),
								matcher.group(3)));
					}
					else {
						throw new InvalidSearchQueryException(String.format("Expression %s did not match regex: %s",
								searchString, SpecCriteriaRegex));
					}
				}
			});
		}
