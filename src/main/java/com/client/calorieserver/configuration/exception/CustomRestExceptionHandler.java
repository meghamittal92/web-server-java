...
/**
 * Custom handler to translate exceptions into Error Response objects. Uses codes and
 * messages defined in {@link ApiError} class to propagate to client.
 */
@ControllerAdvice
public class CustomRestExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleConstraintViolation(final ConstraintViolationException ex,
		...
		for (final ConstraintViolation<?> violation : ex.getConstraintViolations()) {
			details.add(violation.getRootBeanClass().getName() + " " + violation.getPropertyPath() + ": "
	...
	protected ResponseEntity<Object> handleJsonMappingException(final JsonMappingException ex,
			final WebRequest request) {
		logger.error("Exception: {}\n", ex);
		ErrorResponse errorResponse = buildErrorResponse(HttpStatus.BAD_REQUEST, ApiError.INVALID_INPUT);
		return ResponseEntity.status(errorResponse.getStatus()).body(errorResponse);
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
		...
		if (cause instanceof UnrecognizedPropertyException) {
			details = String.format("Unrecognized field %s. Known fields: %s",
					((UnrecognizedPropertyException) cause).getPropertyName(),
					((UnrecognizedPropertyException) cause).getKnownPropertyIds());
		}
	...
	protected ResponseEntity<Object> handleTypeMismatch(final TypeMismatchException ex, final HttpHeaders headers,
		...
		logger.error("Exception: {}\n", ex);
		final String errorDetail = String.format("%s  value for should be of type %s", ex.getValue(),
				ex.getPropertyName(), ex.getRequiredType());

		final ErrorResponse errorResponse = buildErrorResponse(HttpStatus.BAD_REQUEST, ApiError.INVALID_INPUT,
				List.of(errorDetail));
		return handleExceptionInternal(ex, errorResponse, headers, errorResponse.getStatus(), request);
	}
	...
	protected ResponseEntity<Object> handleMethodArgumentTypeMismatch(final MethodArgumentTypeMismatchException ex,
		...
		logger.error("Exception: {}\n", ex);
		final String errorDetail = String.format("% should be of type %s", ex.getName(),
	...
	protected ResponseEntity<Object> handleNoHandlerFoundException(NoHandlerFoundException ex, HttpHeaders headers,
		...
		ErrorResponse errorResponse = buildErrorResponse(HttpStatus.NOT_FOUND, ApiError.HANDLER_NOT_FOUND,
				List.of(errorDetail));
		return handleExceptionInternal(ex, errorResponse, headers, errorResponse.getStatus(), request);
	}
	...
	protected ResponseEntity<Object> handleDisabledException(AccessDeniedException ex,
	...
	protected ResponseEntity<Object> handleDisabledException(BadCredentialsException ex,
		...
		ErrorResponse errorResponse = buildErrorResponse(HttpStatus.UNAUTHORIZED, ApiError.BAD_CREDENTIALS,
				List.of("Incorrect Password"));
		return ResponseEntity.status(errorResponse.getStatus()).body(errorResponse);
	}

	@ExceptionHandler({ InvalidSearchQueryException.class })
	protected ResponseEntity<Object> handleInvalidSearchQueryException(InvalidSearchQueryException ex,
			final HttpServletRequest request) {
	...
	protected ResponseEntity<Object> handleCalorieServiceException(ExternalCalorieServiceException ex,
			final HttpServletRequest request) {
